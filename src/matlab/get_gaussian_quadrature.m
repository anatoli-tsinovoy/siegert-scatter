function [omega_i, x_i] = get_gaussian_quadrature( order, alpha, beta, a, b)
% Modified from this file:
% https://people.sc.fsu.edu/~jburkardt/m_src/jacobi_rule/jacobi_rule.html
kind = 4;
[ x_i, omega_i ] = cgqf ( order, kind, alpha, beta, a, b );
end
...
%
[ aj, bj, zemu, log2zemu ] = class_matrix ( kind, nt, alpha, beta );
%
%  Compute the knots and weights.
%
[ t, wts ] = sgqf ( nt, aj, bj, zemu, log2zemu );
...
function [ aj, bj, zemu, log2zemu ] = class_matrix ( kind, m, alpha, beta )
...
elseif ( kind == 3 )
    ...
    ab = alpha * 2.0;
    log2zemu = ( ab + 1.0 ) + log2(gamma ( alpha + 1.0 )^2 ...
        / gamma ( ab + 2.0 ));
...
elseif ( kind == 4 )
    ...
    ab = alpha + beta;
    abi = 2.0 + ab;
    log2zemu = ( ab + 1.0 ) + gammaln ( alpha + 1.0 )/log(2) + ...
        gammaln ( beta + 1.0 )/log(2) -  gammaln ( abi )/log(2);
...
elseif ( kind == 6 )
    ...
    log2zemu = gammaln ( ( alpha + 1.0 ) / 2.0 )/log(2);
    zemu = 2.0^(log2zemu);
...
function [ t, wts ] = sgqf ( nt, aj, bj, zemu, log2zemu )
